...
| `full_level_map01` | Pendiente definitivo (dos intentos) | Regular/Malo | No aplica | Intento 1 (reward original): `mean_reward` clavado en `-0.420` (`std=0.000`), sin variacion. Intento 2 (reward rebalanceado, `living_reward=-0.001`/`death_penalty=0.3`, 2026-07-16): `mean_reward` oscilo entre `-4.200` y `-1.584` (mejor, step 750000) a lo largo de 20 evaluaciones, sin converger; eval final (1M steps) volvio a `-4.200`/`mean_length=4200` (colapso completo). Nunca hubo señal de `map_exit_reward` en ningun intento. | Piloto de nivel real de Doom (Freedoom 2 MAP01, mas alla de los 9 escenarios curados). El rebalance de reward si cambio el comportamiento (acciones mas diversas: ataca, usa puertas, ya no gira en circulo fijo) pero no logro consolidar una politica estable — sube y colapsa sin tendencia. Arquitectura/pipeline correctos en ambos intentos. No promovido. |
